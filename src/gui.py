...
class GUI(QMainWindow):
    def __init__(self, maze, square_size):
        ...
        self.god = (
            False  # allow frustrated players to enter god_mode, and go through walls
        )
        ...
        self.tip = QtWidgets.QPushButton(
            self
        )  # user can ask for tip: gives 20% of the remaining solution
    ...
    def show_tip(self):
        ...
        msg.setText(
            "Following these directions will help you find home!\n {}".format(tip)
        )
        msg.exec()
        ...
        if not self.quitting:
            ...
            if moving:
                ...
                if self.god:  # allow user to move through walls if in god-mode
                    if (
                        self.maze.square_in_bounderies(square.x + xi, square.y + yi)
                        == True
                    ):
                        square.remove_mouse()
                    ...
                    if not we_are_under:
                        ...
                        if (
                            ...
                            if square.walls[direction]:
                                ...
                                if self.maze.get_square(
                                    square.x + xi, square.y + yi
                                ).has_square_under():
                                    square.remove_mouse()
                                    self.maze.get_square(
                                        square.x + xi, square.y + yi
                                    ).get_under_square().add_mouse()
    ...
    def draw_mouse(self, loc_x, loc_y, mx, my):
        if self.maze.get_square(mx, my).mouse:
            self.mouse.setFont(
                QtGui.QFont("Arial", self.basic_font_size, QtGui.QFont.Bold)
            )
            self.mouse.move(
                loc_x * self.square_size
                + (self.square_size - self.basic_font_size) / 2,
                loc_y * self.square_size
                + (self.square_size - self.basic_font_size) / 2,
            )
        else:
            # demonstrate being under a square with smaller font
            self.mouse.setFont(
                QtGui.QFont("Arial", self.basic_font_size * 0.6, QtGui.QFont.ExtraLight)
            )
            self.mouse.move(
                loc_x * self.square_size
                + (self.square_size - self.basic_font_size * 0.6) / 2,
                loc_y * self.square_size
                + (self.square_size - self.basic_font_size * 0.6) / 2,
            )
    ...
    def drawWalls(self, painter, maze):
        ...
        squares_to_display = int(
            700 / self.square_size
        )  # calculate the amount of squares we can fit on the screen
        ...
        for y in range(
            mouse_square.y - int(squares_to_display / 2),
            mouse_square.y - int(squares_to_display / 2) + squares_to_display,
        ):
            count_x = 0
            for x in range(
                mouse_square.x - int(squares_to_display / 2),
                mouse_square.x - int(squares_to_display / 2) + squares_to_display,
            ):
    ...
    def drawSolution(self, painter, maze, path):
        ...
        for y in range(self.maze.height):
            for x in range(self.maze.width):
                ...
                if square.has_this_wall("N"):
                    painter.drawLine(
                        (x * length), (y * length), (x * length + length), (y * length)
                    )
                ...
                if square.has_this_wall("W"):
                    painter.drawLine(
                        (x * length), (y * length), (x * length), (y * length + length)
                    )
                if square.has_this_wall("E"):
                    painter.drawLine(
                        (x * length + length),
        ...
        self.mouse.move(
            self.maze.width * length - 0.5 * length, self.maze.height * length
        )  # guide mouse to the exit
        ...
        for p in path:
            ...
            painter.drawEllipse(
                length * x + length / 4, length * y + length / 4, length / 2, length / 2
            )
