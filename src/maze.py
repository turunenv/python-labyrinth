...
class Maze:
    ...
    def carve_maze(self):
        ...
        while len(squarelist) != 0:
            ...
            current_square = squarelist[
                len_squarelist - 1
            ]  # select newest square in squarelist
            ...
            if num_options == 0:
                ...
                if len(carving_options) != 0:
                    ...
                    under_square = self.get_square(
                        current_square.x + xb, current_square.y + yb
                    ).add_under_square()
                    squarelist.append(under_square)
                    next_square = self.get_square(
                        current_square.x + xb * 2, current_square.y + yb * 2
                    )
        ...
        for direction, (xi, yi) in Maze.directions:
            if self.square_in_bounderies(square.x + xi, square.y + yi):
                ...
                if self.get_square(square.x + xi, square.y + yi).has_not_been_visited():
                    unvisited.append(
                        (direction, self.get_square(square.x + xi, square.y + yi))
                    )
        ...
        for direction, (xi, yi) in Maze.directions:
            if self.square_in_bounderies(square.x + xi, square.y + yi):
                ...
                if (
                    ...
                    and self.get_square(square.x + xi, square.y + yi).vertical_passage()
                ) or (
                    ...
                    if self.square_in_bounderies(square.x + xi * 2, square.y + yi * 2):
                        if self.get_square(
                            square.x + xi * 2, square.y + yi * 2
                        ).has_not_been_visited():
                            # don't carve into dead ends
                            if (
                                len(
                                    self.unvisited_neighbours(
                                        self.get_square(
                                            square.x + xi * 2, square.y + yi * 2
                                        )
                                    )
            ...
            for direction in ["N", "S", "W", "E"]:
                ...
                if self.validMove(put):
                    ...
                    if len(add) == 0:
                        ...
                        not add[len(add) - 1] == Square.walls_between_squares[direction]
                    ):
                        paths.put(put)
        return add
    ...
    def validMove(self, path):
        ...
        if (
            len(path) > 1
            and self.get_square(x2, y2).walls[
                Square.walls_between_squares[path[len(path) - 2]]
            ]
            and self.get_square(x2, y2).has_square_under()
        ):
    ...
    def save_to_file(self, filename):
        ...
        f.write(
            "{}/{}/{}\n".format(x, y, self.mouse)
        )  # mark labyrinth width,height and mouse_symbol to the first row
        ...
        for a in range(y):
            for b in range(x):
                ...
                for direction in [
                    ...
                    "S",
                    "W",
                ]:  # write squares to file, marking walls with '1's, no_walls with 0's, going clockwise
        ...
        if self.get_square(
            x1, y1
        ).mouse:  # check if mouse is on the surface or on the possible under-square and mark accordingly -> S=surface, U=under
            f.write("S/{}/{}/\n".format(x1, y1))
        else:
            f.write("U/{}/{}/\n".format(x1, y1))
    ...
    def load_from_file(cls, filename):
        ...
        if not isinstance(x, int) and not isinstance(y, int):
            raise CorruptedMazeFileError(
                "Given measures for width and height not integers!"
            )
        else:
            maze = Maze(x, y, mouse)
            for y1 in range(y):
                ...
                for x1 in range(x):
                    for s in range(
                        6
                    ):  # every square in the file contains 5 characters plus the separator, i.e. '1110F/'
                        if s == 5:
                            if line[x1 * 6 + s] != "/":
                                raise CorruptedMazeFileError(
                                    "Missing separator from squares!"
                                )
                        elif s == 4:
                            if line[x1 * 6 + 4] != "F" and line[x1 * 6 + 4] != "T":
                                raise CorruptedMazeFileError(
                                    "Incorrect symbol for marking undersquare!"
                                )
                            elif line[x1 * 6 + 4] == "T":
                            ...
                            if line[x1 * 6 + s] != "1" and line[x1 * 6 + s] != "0":
                                raise CorruptedMazeFileError(
                                    "Incorrect symbol in marking walls!"
                                )
